...
const NavbarMenu: React.FC<Menu> = (props: Menu) => {
  ...
  React.useLayoutEffect(() => window.scrollTo({top: 0, left: 0, behavior: 'smooth'}), [pathname]);
...
const Navbar: React.FC<any> = (props: NavbarProps) => {
  ...
  return (
    <div>
      <div className="navbar shadow-lg border-2 glass absolute m-auto z-50 text-white h-16 box-border"> {/* bg-base-100 hex2 */}
        <div className="navbar-start">
          ...
          <Link className="btn btn-ghost normal-case text-xl gold-text" to={`/home`}>{props.brand || "Brand"}</Link>
      ...
      {/* <div className={`h-16 ${ storageContext.theme?.card2color || 'bg-slate-900' }`}></div> */}
